...
export default function ClientUi({ res }: { res: StatsData }) {
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="stat-card border-none bg-secondary/30 shadow-none hover:bg-secondary/50 transition-colors">
            ...
            <CardContent>
              <div className="text-3xl font-bold">
                {formatFileSize(res?.totalSize || 0)}
              </div>
          ...
          <Card className="stat-card border-none bg-secondary/30 shadow-none hover:bg-secondary/50 transition-colors">
            ...
            <CardContent>
              <div className="text-3xl font-bold">{res?.avgTime}ms</div>
              <p className="text-xs text-muted-foreground mt-1">
                Per conversion unit
              </p>
            </CardContent>
          ...
          <Card className="stat-card border-none bg-secondary/30 shadow-none hover:bg-secondary/50 transition-colors">
            ...
            <CardContent>
              <div className="text-3xl font-bold uppercase">
                {res?.formats[0]?.type.includes("/")
                  ? res.formats[0].type.split("/")[1]
                  : res?.formats[0]?.type || "None"}
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                Most used target
              </p>
                ...
                <div key={f.type} className="group">
                  ...
                  <div className="w-full bg-secondary rounded-full h-2 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(f.count / res.totalFiles) * 100}%`,
                        opacity: 1 - i * 0.15,
                      }}
                    ></div>
                  </div>
